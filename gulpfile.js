...
gulp.task('clean-css', () => {
  return gulp.src('src/css/styles.css').pipe(clean());
});
...
gulp.task('browserSync', ['sass'], () => {
  var filesToWatch = ['src/sass/**/*.scss', 'index.html', 'src/js/**/*.js'];
...
gulp.task('serve', ['sass', 'browserSync'], function() {
  gulp.watch('src/sass/**/*.scss', ['sass', 'idd']);
...
gulp.task('copy-assets', function() {
  ...
  merge(assets, favicon).pipe(gulp.dest(files.out.assets));
});
